...
<BODY>
	<FORM id=form1 name=form1
		action="${pageContext.request.contextPath }/spring/crm/updateUser"
		method=post>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							<TR>
								...
								<INPUT class=textbox id=sChannel2
											style="WIDTH: 180px" maxLength=50 name="username" value="${user.username }" readonly="readonly">
								</td>
							...
							<TR>
								<td>选择是否为超级用户：</td>
								<td>
								<input type="radio" name="status" value="1" <c:if test="${user.status eq 1}">checked="checked"</c:if>>是
								<input type="radio" name="status" value="0" <c:if test="${user.status eq 0}">checked="checked"</c:if>>不是
								</td>
